...
method.placeReservation = function(json_input, callBack){
    var con = this.connection;
    var sqlsearch = "SELECT * FROM RESERVATIONS WHERE DESCRIPTION = '" + json_input.desc + "'";
...
method.setTable = function(json_info,callBack){
    ...
    con.query(searchsql, function(err,rows){
        ...
        if(rows.length>0){
            var updatesql = "UPDATE TABLES SET X_COORD = " + json_info.xcoord + ", Y_COORD = " + json_info.ycoord + ", NUM_SEATS = " + json_info.number_of_seats + ", STATUS = '" + json_info.stat + "', EMPLOYEE_ID = " + json_info.employeeID
 + " WHERE NAME = '" + json_info.name + "'";
            console.log(updatesql);
            con.query(updatesql, function(err,rows){
                if(err){
                    throw err;
...
method.getLogEvent = function(json_info,callBack){
    ...
    connection.query(sql,function(err,rows){
        ...
        if(rows.length>0 && json_info.index < rows.length){
...
method.getFoodByID = function(json_info,cb){
    var food = json_info.food;
    parseFID(this.connection,0,food,function(res){
        return cb(res);
    });
 ...
 */
function parseFID(connection,index,foodIDS,cb){
    console.log(foodIDS);
    var con = connection;
    var cache = "";
    var json = {};
    ...
    var idlist = {};
    var count = 0;
    for(x=0;x<foodIDS.length;x++){
	console.log(x);
	if(foodIDS.charAt(x)==="-"){	
	 var foodData = processSegment(cache);
	idlist[count] = foodData;
	count++;
	cache = "";
	}else{
